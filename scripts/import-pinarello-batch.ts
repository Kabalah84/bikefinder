...
import { parsePinarelloHtml } from "./parse-pinarello";
...
const PINARELLO_LINKS_FILE = path.join(process.cwd(), "links", "pinarello.txt");
const CACHE_DIR = path.join(process.cwd(), "scratch", "pinarello_cache");

interface PinarelloItem {
  url: string;
...
function readPinarelloLinks(): PinarelloItem[] {
  const content = fs.readFileSync(PINARELLO_LINKS_FILE, "utf-8");
  const lines = content.split("\n");

  let currentSection: "carretera" | "gravel" | "montana" = "carretera";
  const items: PinarelloItem[] = [];
  ...
  for (const line of lines) {
    ...
    if (trimmed.startsWith("#")) {
      ...
      if (lower.includes("carretera") || lower.includes("endurance")) {
...
async function run() {
  console.log("🚲 ========================================================");
  console.log("   IMPORTADOR POR LOTES: PINARELLO OFICIAL");
  console.log("========================================================\n");

  const items = readPinarelloLinks();
  console.log(`📋 Total de enlaces Pinarello detectados: ${items.length}`);
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    try {
      const bike = await parsePinarelloHtml(html, url, section);
      if (!bike) {
  ...
  console.log("\n========================================================");
  console.log(`🏁 FIN DEL PROCESAMIENTO PINARELLO`);
