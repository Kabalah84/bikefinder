...
import { parseMegamoHtml } from "./parse-megamo";
...
const MEGAMO_LINKS_FILE = path.join(process.cwd(), "links", "megamo.txt");
const CACHE_DIR = path.join(process.cwd(), "scratch", "megamo_cache");

interface MegamoItem {
  url: string;
...
function readMegamoLinks(): MegamoItem[] {
  const content = fs.readFileSync(MEGAMO_LINKS_FILE, "utf-8");
  const lines = content.split("\n");

  let currentSection: "carretera" | "gravel" | "montana" = "carretera";
  const items: MegamoItem[] = [];
...
async function run() {
  console.log("🚲 ========================================================");
  console.log("   IMPORTADOR POR LOTES: MEGAMO OFICIAL");
  console.log("========================================================\n");

  const items = readMegamoLinks();
  console.log(`📋 Total de enlaces Megamo únicos detectados: ${items.length}`);
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    try {
      const bike = await parseMegamoHtml(html, url, section);
      if (!bike) {
  ...
  console.log("\n========================================================");
  console.log(`🏁 FIN DEL PROCESAMIENTO MEGAMO`);
