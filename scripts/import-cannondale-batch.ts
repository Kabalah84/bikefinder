...
import { parseCannondaleHtml } from "./parse-cannondale";
...
const CANNONDALE_LINKS_FILE = path.join(process.cwd(), "links", "cannondale.txt");
const CACHE_DIR = path.join(process.cwd(), "scratch", "cannondale_cache");

interface CannondaleItem {
  url: string;
...
function readCannondaleLinks(): CannondaleItem[] {
  const content = fs.readFileSync(CANNONDALE_LINKS_FILE, "utf-8");
  const lines = content.split("\n");

  let currentSection: "carretera" | "gravel" | "montana" = "carretera";
  const items: CannondaleItem[] = [];
...
async function run() {
  console.log("🚲 ========================================================");
  console.log("   IMPORTADOR POR LOTES: CANNONDALE OFICIAL");
  console.log("========================================================\n");

  const items = readCannondaleLinks();
  console.log(`📋 Total de enlaces Cannondale únicos detectados: ${items.length}`);
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    try {
      const bike = await parseCannondaleHtml(html, url, section);
      if (!bike) {
  ...
  console.log("\n========================================================");
  console.log(`🏁 FIN DEL PROCESAMIENTO CANNONDALE`);
