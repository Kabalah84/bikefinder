...
import { parseMeridaHtml } from "./parse-merida";
...
const MERIDA_LINKS_FILE = path.join(process.cwd(), "links", "merida.txt");
const CACHE_DIR = path.join(process.cwd(), "scratch", "merida_cache");

interface MeridaItem {
  url: string;
...
function readMeridaLinks(): MeridaItem[] {
  const content = fs.readFileSync(MERIDA_LINKS_FILE, "utf-8");
  const lines = content.split("\n");

  let currentSection: "carretera" | "gravel" | "montana" = "carretera";
  const items: MeridaItem[] = [];
  ...
  for (const line of lines) {
    ...
    if (trimmed.startsWith("#")) {
      ...
      } else if (lower.includes("gravel") || lower.includes("all-road") || lower.includes("ciclocross")) {
...
async function run() {
  console.log("🚲 ========================================================");
  console.log("   IMPORTADOR POR LOTES: MERIDA OFICIAL");
  console.log("========================================================\n");

  const items = readMeridaLinks();
  console.log(`📋 Total de enlaces Merida únicos detectados: ${items.length}`);
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    try {
      const bike = await parseMeridaHtml(html, url, section);
      if (!bike) {
  ...
  console.log("\n========================================================");
  console.log(`🏁 FIN DEL PROCESAMIENTO MERIDA`);
